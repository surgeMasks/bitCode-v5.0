<script>
  // Additional logic can be added here if necessary
</script>

<main class="flex items-center justify-center min-h-screen bg-red-50">
  <div class="text-center p-8 bg-white shadow-lg rounded-lg">
    <svg class="mx-auto mb-4 w-16 h-16 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
    </svg>
    <h1 class="text-3xl font-bold text-gray-800 mb-2">Payment Canceled</h1>
    <p class="text-gray-600 mb-6">Your payment was canceled. If you have any questions, please contact our support team.</p>
    <a href="/checkout" class="inline-block px-6 py-2 text-white bg-red-500 hover:bg-red-600 rounded-full">Return to Checkout</a>
  </div>
</main>
